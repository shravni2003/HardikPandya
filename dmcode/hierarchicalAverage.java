...
public class hierarchicalAverage {
    ...
    public static double averageLinkage(double[][] distMatrix, List<Integer> cluster1, List<Integer> cluster2) {
        double sum = 0;
        for (int i : cluster1) {
            for (int j : cluster2) {
                sum += distMatrix[i][j];
    ...
    public static int[] findNearestClusters(double[][] distMatrix, List<List<Integer>> clusters) {
        ...
        for (int i = 0; i < clusters.size(); i++) {
            for (int j = i + 1; j < clusters.size(); j++) {
                double distance = averageLinkage(distMatrix, clusters.get(i), clusters.get(j));
                if (distance < minDist) {
                    minDist = distance;
                    first = i;
                    second = j;
    ...
    public static void clusterAnalysis(List<Double> values, int targetClusters) {
        ...
        while (clusters.size() > targetClusters) {
            ...
            System.out.println("\nCombining clusters: " + c1 + " and " + c2 
                    + ", distance: " + averageLinkage(distMatrix, clusters.get(c1), clusters.get(c2)) + "\n");
